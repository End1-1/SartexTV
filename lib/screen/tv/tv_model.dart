...
class TVModel {
  final streamController = StreamController<List<ModelRow>>.broadcast();
  final viewController = StreamController<String?>.broadcast();
  var fontSize = 30.0;
  ...
  _showData() {
    ...
    pageNumCount = prefs.getInt(key_tv_page_count) ?? 8;
  ...
  Future<void> _refreshData() async {
    final e = await HttpSqlQuery.post({"sl":"select status, message, fontsize from messages where branch='${prefs.branch()}'"});
    if (e.isNotEmpty && e[0]['status'] == '1') {
      fontSize = double.tryParse(e[0]['fontsize'] ?? '') ?? 30;
      viewController.add(utf8.decode(base64.decode(e[0]['message'] ?? '')));
      return;
    }
